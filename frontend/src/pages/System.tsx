...
const heat = (reading?: Temperature) => {
  if (!reading?.celsius && reading?.celsius !== 0) return "text-zinc-400";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <h1 className="text-sm uppercase tracking-wide text-zinc-400">System</h1>
        {units.map((nas) => (
          <button
            key={nas.id}
      ...
      {!busy && units.length > 0 && (
        <div className="grid lg:grid-cols-2 gap-4">
          <div className="card space-y-2">
            <h2 className="text-xs uppercase tracking-wide text-zinc-500">The box</h2>
            <div className="text-sm">
              {info.model?.display_model_name ?? "—"}
              <span className="text-zinc-500"> · {info.model?.hostname}</span>
            </div>
            <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-zinc-400">
              ...
              {info.model?.serial_number && (
                <span className="text-zinc-500">serial {info.model.serial_number}</span>
              )}
            </div>
          ...
          <div className="card space-y-2">
            <h2 className="text-xs uppercase tracking-wide text-zinc-500">Load</h2>
            ...
            {latest && (
              <div className="text-xs text-zinc-400">
                ...
                {cores > 0 && (
                  <span
                    className={
                      (latest.load_1min ?? 0) > cores ? "text-amber-400" : "text-zinc-500"
                    }
                  >
                    {" "}
                    over {cores} cores
            ...
            <div className="text-xs text-zinc-400">
              ...
              <span className="text-zinc-600"> · {mb(info.memory?.cache_mb)} cache</span>
            </div>
          ...
          <div className="card space-y-2">
            <h2 className="text-xs uppercase tracking-wide text-zinc-500">Disks</h2>
            <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs">
              {disks.map((disk) => (
                <span
                  key={disk.id}
                  className={disk.temp_alert ? "text-red-400" : "text-zinc-400"}
                >
                  bay {disk.id} {disk.temperature_celsius} °C
                  {disk.is_ssd && <span className="text-zinc-600"> SSD</span>}
                </span>
              ))}
              {disks.length === 0 && <span className="text-zinc-500">No disks reported.</span>}
            </div>

            <h2 className="text-xs uppercase tracking-wide text-zinc-500 pt-2">Power schedule</h2>
            {power.entries?.length ? (
              power.entries.map((entry) => (
                <div key={entry.entry_id} className="text-xs text-zinc-400">
                  {entry.action} {entry.day_pattern} at{" "}
                  {String(entry.hour).padStart(2, "0")}:{String(entry.minute).padStart(2, "0")}
                  <span className={power.schedule_enabled ? "text-zinc-500" : "text-zinc-600"}>
                    {power.schedule_enabled ? " · schedule on" : " · schedule off"}
                  </span>
                </div>
              ))
            ) : (
              <div className="text-xs text-zinc-500">Nothing scheduled.</div>
            )}
          </div>

          <div className="card space-y-2">
            <h2 className="text-xs uppercase tracking-wide text-zinc-500">Network</h2>
            {(info.network?.interfaces ?? []).map((nic) => (
              <div key={nic.name} className="flex flex-wrap items-center gap-x-4 text-xs">
                <span className={nic.status ? "text-zinc-300" : "text-zinc-600"}>
                  {nic.name}
                </span>
                <span className="text-zinc-500">{nic.display_name}</span>
                <span className="text-zinc-400">{nic.ip_address || "no address"}</span>
                {nic.max_speed_mbps ? (
                  <span className="text-zinc-600">{nic.max_speed_mbps} Mb/s</span>
                ) : null}
                {!!nic.error_packets && (
          ...
          <div className="card space-y-2 lg:col-span-2">
            ...
            <div className="grid md:grid-cols-2 gap-x-12">
              {(apps.apps ?? []).map((app) => (
                ...
                >
                  <span className={app.enabled ? "text-zinc-300 flex-1" : "text-zinc-600 flex-1"}>
                    {app.displayName}
                  </span>
                  {!app.enabled && <span className="text-zinc-600">disabled</span>}
                  {app.updateAvailable && <span className="text-amber-400">update</span>}
                  <span className="text-zinc-500">{app.version}</span>
                </div>
              ))}
              {(apps.apps ?? []).length === 0 && (
                <span className="text-xs text-zinc-500">No applications reported.</span>
              )}
            </div>
          </div>
        </div>
